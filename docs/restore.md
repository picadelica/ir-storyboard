...
- Куда ложится:
  ...
  - **офсайт — Яндекс.Диск** через `rclone`, `yandex:prod-backups/ir-storyboard`
    ...
    **Работает — проверено 2026-08-30 по логам прогонов:** `CLOUD: rclone copy …
    → yandex:prod-backups` и `CLOUD: ok` в каждом из последних прогонов (~2 мин
    на 7.8 ГБ инкрементально). Значит `secrets/rclone.conf` на сервере есть и
    копии реально уезжают с машины. Если в логе появится
    `CLOUD: WARN … ПРОПУЩЕН` — конфиг пропал, и ВСЕ копии снова живут на одном
    боксе (инструкция по заливке — `docs/backup.md` оркестратора).
...
> **Про `-v storyboard-data:/data`.** Настоящее имя тома —
> `ir-storyboard_storyboard-data`: compose добавляет префикс проекта. Короткое имя
> в `docker compose run -v` при этом РАБОТАЕТ — Compose v2 разрешает его в том же
> объявленный в файле том (проверено 2026-09-01 на Docker 29.7.2: файл, записанный
> через `/app/data`, виден в `/data`, том один). Но такая команда монтирует один и
> тот же том дважды по двум путям и держится на неочевидном правиле разрешения имён.
> Ниже — явный путь `/app/data` через сам сервис: короче, однозначнее и не зависит
> от версии compose.
...
2. Посмотреть **лог** последнего ночного `backup_prod` — не статус (см. врезку
   ниже о том, почему статус врёт). Ищем две строки: `ir-storyboard: <размер>`
   (копия за эту ночь снята) и `CLOUD: ok` (уехала наружу). `CLOUD: WARN …
   ПРОПУЩЕН` означает, что все копии лежат на одном боксе — тогда залить
   `secrets/rclone.conf` (инструкция — «Настройка облака» в `docs/backup.md`
   оркестратора). Достать лог, не заходя на сервер:
   ```bash
   source ~/.config/conductor-orchestrator/credentials
   curl -s -u "$GATEWAY_AUTH" \
     "$GATEWAY_URL/workflow/search?query=workflowType%3D%27backup_prod%27&size=5&sort=startTime:DESC"
   curl -s -u "$GATEWAY_AUTH" "$GATEWAY_URL/workflow/<workflowId>"   # tasks[].outputData.stdout
   ```
   (нужен ssh-туннель до гейта — см. `NEXT.md`, «Доступ на прод».)
...
- **Учение проведено 2026-09-01 — восстановление работает.** Прогнан полный цикл
  на локальной копии, прод не трогали: база с пилотными данными (54 факта, 24
  ячейки) → снапшот тем же способом, что и в проде (online-backup API + gzip) →
  авария (массовое удаление фактов) → восстановление → **счётчики совпали**.
  Отдельно проверено, что документированная команда готовности
  (`./scripts/check_migration.sh <файл>`) отрабатывает как написана, в том числе
  на системном python без venv.

- **Поправка к записи от 2026-08-30: `scripts/restore.sh` НЕ был сломан.** Тогда
  было записано, что `-v storyboard-data:/data` монтирует новый пустой том и
  восстановление молча уходит в пустоту. Это неверно: Compose v2 разрешает короткое
  имя в объявленный в файле том проекта, том получался один и тот же (проверено
  прямым тестом — см. врезку в сценарии A). Старый скрипт восстанавливал базу
  корректно. Правка от 30.08 остаётся в силе, но это **читаемость и устойчивость**
  (явный путь вместо неявного разрешения имён, плюс домиграция схемы и счётчики
  после заливки), а не починка отказа.

- **Копии реально снимаются и реально уезжают в облако.** Прочитаны логи
  последних ночных прогонов: `ir-storyboard: online-снимок matrix.db` →
  `архив файлов /app/data` → `195.7M` каждую ночь, затем `CLOUD: ok`. То есть
  офсайт живой, и «все копии на одной машине» — уже НЕ про нас.

> ### ⚠️ Красный ночной бэкап ≠ «нас не забэкапили»
>
> С 2026-08-27 workflow `backup_prod` завершается со статусом FAILED четыре ночи
> подряд. **Причина не в ir-storyboard.** В логе:
> `orbita: стенд на Mac, а свежего (≤48ч) дампа в /root/backups/orbita-stand НЕТ`
> → `ПОКРЫТИЕ: у проекта 'orbita' НЕТ копии` → `ЗАВЕРШЕНО С ОШИБКАМИ`. Скрипт
> честно краснеет за чужой непокрытый проект и при этом доводит до конца всё
> остальное: наш снимок и архив снимаются, облако отрабатывает.
>
> Практический вывод для аварии: **смотреть в лог прогона, а не на его статус.**
> Строка `ir-storyboard: <размер>` — вот что означает «копия за эту ночь есть».
> (И наоборот: хроническая краснота из-за соседнего проекта приучает не смотреть
> на статус вообще — это стоит починить на стороне orbita, а не терпеть.)
...
- **Само учение по восстановлению ни разу не проводилось** — ни один из трёх
  ...
  есть (vitalis, medmodel-lab — 2026-08-12), у этого нет. Это теперь главный
  открытый риск документа: механика описана и вычитана, но не испытана.
