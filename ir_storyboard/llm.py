...
Ты видишь ТОЛЬКО метаданные (заголовок, ссылку, описание, площадку) — расшифровки нет.

Нам нужны ВЫСТУПЛЕНИЯ: интервью, подкаст, доклад, эфир, панель, лекция, AMA — то есть
запись, где нужный человек ГОВОРИT сам, своими словами, достаточно долго, чтобы из этого
вышли факты для матрицы.

Проверяй ДВА условия независимо, и оба должны выполниться для "likely":
1. ЧЕЛОВЕК — это действительно нужный человек (или его компания), а не тёзка.
2. ФОРМА — это запись речи, а не текст о человеке.

Что формой НЕ является (даже если человек тот самый — это "unlikely"):
- профиль и карточка человека: LinkedIn, Facebook, страница спикера конференции,
  «Co-Founder & COO» в заголовке без признаков разговора, каталоги и справочники;
- страницы компании, продуктовые лендинги, вакансии, whitepaper и PDF-документы;
- пресс-релизы, новости и статьи ПРО человека или компанию, написанные третьими лицами
  («X привлёк $46M», «почему мы инвестировали в Y»), — даже с парой цитат;
- обзоры рынка и нарезки, где нужный человек лишь упоминается.

Ссылка — сильный сигнал о форме: /watch, /episode, /podcast, /shorts, spotify, youtube,
vk video — обычно речь; linkedin.com/in/, facebook.com/, /team, /speakers, /careers,
.pdf — обычно нет.

relevance:
- "likely"   — оба условия выполнены: это запись речи нужного человека;
- "unclear"  — форма похожа на разговор, но не видно, говорит ли нужный человек (либо
               наоборот: человек тот, а формат по метаданным не разобрать);
- "unlikely" — тёзка/чужая тема ЛИБО это не запись речи (профиль, статья, релиз, документ).
...
- Цитаты дословные: в was — из прошлого обзора (с датой), в now — из нового (с таймкодом).
  Если у прошлого выступления дата неизвестна — оставь was.date пустым, НЕ подставляй
  сегодняшнюю и не выдумывай.
...
def compare_with_previous_digests(new_digest: dict, previous: List[dict],
    ...
    for p in previous[-10:]:
        when = (p.get("date") or "").strip() or "дата неизвестна"
        prev_lines.append(f"\n— ВЫСТУПЛЕНИЕ {when}: {(p.get('main_motif') or '')[:600]}")
