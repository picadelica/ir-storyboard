...
- **Research** — два входа. Своя ссылка: страница забирается в текст (Tavily extract, без ключа — прямой HTTP + вычистка HTML на stdlib), текст показывается аналитику до классификации; YouTube-ссылка уводится в свой ингест, уже разобранная — помечается. Либо веб-поиск (Tavily) по сгенерированным запросам. Дальше у обоих общий путь: превью и коммит.
- **Мониторинг** — см. ниже.
