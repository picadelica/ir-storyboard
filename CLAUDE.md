...
## Текущий workspace: дизайн/UX-форк Picadelica для будущего подключения к upstream

Этот локальный checkout сейчас работает как дизайн/UX-вариант основного проекта:

- `origin` = `https://github.com/picadelica/ir-storyboard.git`.
- `upstream` = `https://github.com/dechernov-stack/ir-storyboard.git` после
  подключения доступа к основному проекту.
- Рабочая ветка = `main`.
- Цель форка = сохранить новый дизайн и UX-путь обработки входящих данных как
  альтернативный/дублирующий вариант интерфейса, который сможет подключиться к
  основному проекту, его БД, ключам и production-контуру.
- Оригинальный upstream-контекст от основного разработчика описывает контур
  `dechernov-stack/ir-storyboard`, ветку `feat/v2`, прод-сервер и деплой через
  оркестратор. Это карта подключения к основному проекту, но не разрешение
  самостоятельно переключать remote, деплоить production или менять доступы.
- Если пользователь просит «пуш» — пушить в `origin/main` форка Picadelica,
  пока он явно не скажет обратное.
- Приоритет совместимости: не менять схему БД, seed/data-файлы, backend-контракты
  и смысл API без отдельного явного решения. Новый UX должен по возможности
  работать поверх существующих API основного проекта.
- После получения доступа основной репозиторий добавлен как `upstream`.
  Перед переносом в основной проект сравнить `origin/main` с актуальной
  upstream-веткой и отделить frontend-only изменения от рискованных
  backend/data-изменений.

При конфликте между этим разделом и старым описанием production-контура ниже
для текущего workspace приоритет у этого раздела.
...
- **Branch:** в оригинальном проекте — `feat/v2`; в текущем форке Picadelica —
  `main` (см. раздел «Текущий workspace» выше).
...
- Авторизация и multi-user (Basic Auth в Caddy при необходимости).
- Real-time collab editing (last-write-wins).
- Мобильный layout (desktop-only).
- Audit log diff-вью (история через `captured_at`).
...
## Оркестратор оригинального проекта
...
Для текущего форка Picadelica этот раздел не является инструкцией к деплою.
Не запускать production workflow и не трогать сервер без отдельного явного
