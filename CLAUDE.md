...
- **Branch:** `feat/v2` — интеграционная ветка, **из неё и только из неё идёт
  деплой**. Значит всё, что в неё попало, считается готовым к выкатке: пушить
  туда незелёное или незаконченное нельзя. Задачу вести в короткоживущей
  `feat/v2-<тема>`, перед вливанием — `git fetch && git rebase origin/feat/v2`,
  вливать `--ff-only`. `main` — зеркало `feat/v2` (нужен, чтобы GitHub показывал
  свежий README), вручную не трогать. Перед любым push — `git divergence`; при
  расхождении **rebase**, не force и не merge вслепую (глобальный pre-push-хук
  блокирует push при расхождении). Подробности и почему так — `docs/onboarding-dev.md`, §7.
...
- `docs/onboarding-dev.md` — онбординг второго разработчика: уровни доступа,
  генерация ключей (GitHub + узкий SSH-ключ под туннель к оркестратору), локальная
  среда, процедура деплоя, правила работы вдвоём. Тоже живой документ.
