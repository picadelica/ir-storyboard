...
- ~~Нет авторизации.~~ **Устарело.** Авторизация появилась: гейт `_auth_gate` в
  `backend/main.py` закрывает весь `/api/*`, кроме `/api/health` и `/api/auth/*`,
  сессионной кукой `ir_session` от центрального Telegram-шлюза (`AUTHGW_URL` +
  `SESSION_SECRET`, подпись проверяется локально, TTL 21 день; супер-админы —
  `IR_ADMIN_TIDS`). Без этих переменных гейт выключен — так идут локальная
  разработка и тесты. Basic Auth в Caddy не ставился и не нужен; `401` от `/api`
  без логина — нормальный ответ живого маршрута.
